...
const Gallery = () => {
  ...
  const breadcrumbItems = [
    { name: "Accueil", url: "/" },
    { name: t("nav.films", "Films"), url: "/films" }
  ];
          ...
          {topMovies.length > 0 && (
            <div className="mt-6 md:mt-10">
              <p className="text-white/90 font-black uppercase tracking-widest text-xs md:text-sm mb-6">
                {t("gallery.top_movies_subtitle", "Découvrez les 5 meilleurs films")}
              </p>
              <div className="relative h-44 md:h-56 flex items-center justify-center">
                {topMovies.map((movie, index) => {
                  const positionIndex = (index - carouselIndex + topMovies.length) % topMovies.length;
            ...
            <div className="flex flex-col items-center gap-8 mb-16">
              <div className="relative w-full max-w-2xl" ref={searchRef}>
                <div className="relative">
                  ...
                  <input type="text"
                    placeholder={t("gallery.search_placeholder", "Rechercher un film...")}
                    value={searchQuery}
                  ...
                  <div className="absolute z-[100] w-full mt-2 bg-white rounded-2xl shadow-2xl border border-slate-100 overflow-hidden">
                    {suggestions.map((movie) => (
                      <Link key={movie.id} to={`/movie/${movie.id}`}
                        ...
                        <img src={movie.img} alt="" className="w-16 h-9 object-cover rounded-lg shadow-md" />
                        <div>
                          <p className="font-black text-blue-950 text-sm uppercase tracking-tighter">{movie.title}</p>
