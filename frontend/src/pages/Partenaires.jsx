...
export default function Partenaires() {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const data = await getPageBySlug("partenaires");
        if (!cancelled) {
          setPage(data);
          setLoading(false);
        }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-slate-100">
        <div className="text-center">
          ...
          <p className="text-slate-600 text-lg font-medium">Chargement des partenaires…</p>
        </div>
      </div>
    );
  }
      ...
      <div className="bg-gradient-to-r from-blue-600 via-blue-500 to-blue-700 py-16 shadow-xl">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="mt-4 text-lg text-blue-50 max-w-2xl mx-auto">
            Découvrez les partenaires qui accompagnent notre projet
          </p>
        </div>
      </div>

      {/* Contenu WordPress avec design amélioré */}
