...
// --- Helpers ---
const createSlug = (text) => text?.toString().toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim().replace(/\s+/g, "-").replace(/[^\w\-]+/g, "").replace(/\-\-+/g, "-") || "";

const parseJuryData = (html) => {
  if (typeof window === "undefined") return [];
  return Array.from(new DOMParser().parseFromString(html, 'text/html').querySelectorAll('li'))
    .map(li => {
...
const GavelIcon = () => (
  <div className="mb-8 p-5 bg-[#1e293b]/50 backdrop-blur-md rounded-full shadow-[0_0_30px_rgba(37,99,235,0.2)] border border-white/10 ring-1 ring-blue-500/30">
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-10 h-10 text-blue-400">
...
export default function JuryWpage({ page }) {
  ...
  const openArticle = useCallback(async (slug) => {
    if (!slug) return;
    if (articleCache.has(slug)) return setSelected(articleCache.get(slug));

    setLoading(true);
    setSelected({ title: { rendered: 'Chargement...' }, content: { rendered: '' }, isLoading: true });
    
    if (abortRef.current) abortRef.current.abort();
    abortRef.current = new AbortController();

    try {
      const res = await fetch(`/wp-json/wp/v2/posts?slug=${encodeURIComponent(slug)}&_fields=id,title,content,excerpt`, { signal: abortRef.current.signal });
      const data = await res.json();
      if (data?.[0]) {
        articleCache.set(slug, data[0]);
        setSelected(data[0]);
      }
  ...
  useEffect(() => {
    if (selected && scrollRef.current) setTimeout(() => scrollRef.current.scrollIntoView({ behavior: "smooth", block: "start" }), 100);
  }, [selected]);

  const members = useMemo(() => parseJuryData(page?.content?.rendered || ""), [page]);
  const title = page?.title?.rendered || "Le Jury";
  ...
  return (
    <main className="min-h-screen w-full bg-[#0f172a] text-white font-['Montserrat'] flex flex-col items-center py-20 px-4 relative overflow-x-hidden selection:bg-[#38bdf8] selection:text-[#0f172a]">
      ...
      {/* Texture Grain pour l'accessibilité visuelle */}
      ...
      {/* Header - Espacement Mobile First et contrastes élevés */}
      <div className="relative z-10 text-center mb-16 max-w-4xl flex flex-col items-center pt-10">
        <GavelIcon />
        <h1 className="text-5xl md:text-7xl font-black text-white mb-6 tracking-tighter drop-shadow-2xl uppercase" 
      ...
      {/* Article Detail - Design "Card" éclairci */}
      {selected && (
        <div ref={scrollRef} className="relative z-20 w-full max-w-4xl mb-24 animate-in fade-in slide-in-from-bottom-8 duration-500">
          <div className="bg-[#1e293b] rounded-[2.5rem] overflow-hidden shadow-[0_20px_60px_rgba(0,0,0,0.6)] border border-[#334155]">
            <div className="bg-[#1e293b] p-8 md:p-10 border-b border-white/5 flex justify-between items-start">
              <div>
                <span className="text-[#38bdf8] font-black uppercase text-[10px] tracking-[0.2em] mb-3 block">
                  {selected.isLoading ? "Chargement..." : "Profil Jury"}
                </span>
                <h2 className="text-3xl md:text-4xl font-black text-white leading-none uppercase tracking-tight" 
            ...
            <div className="bg-[#0f172a]/30 p-6 text-center border-t border-white/5">
              <button onClick={() => setSelected(null)} className="text-[10px] uppercase tracking-[0.2em] font-black text-[#94a3b8] hover:text-[#38bdf8] transition-colors">
                Fermer le profil
              </button>
            </div>
          </div>
      ...
      {/* Grid - Centrée avec marges larges sur les côtés */}
      <div className="relative z-10 w-full max-w-5xl px-4 md:px-10">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 justify-items-center">
          {members.map((m) => {
            const isSelected = selected && !selected.isLoading && selected.slug === m.slug;
            const isDimmed = selected && !isSelected;
            
            return (
              <div key={m.slug} onClick={() => openArticle(m.slug)}
